...
- **Fase 10 (account deletion):** ✅ **HECHO** (al 2026-06-26). Mutation `deleteAccount` en backend (`core/account_deletion.py`; la mutation vive en `core/schema_mutations.py` tras el refactor del schema) y borrado con **doble confirmación** en la app (`(more)/profile.tsx`, `DELETE_ACCOUNT`). Cumple el requisito de Apple. (El "polish" general sigue siendo trabajo continuo.)
